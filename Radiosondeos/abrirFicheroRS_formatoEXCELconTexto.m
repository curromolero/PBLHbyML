function [error, valoresSondeo, attGlobales] = abrirFicheroRS_formatoEXCELconTexto(direccionFichero)
...
indicesDWPTPositiva = find(valoresSondeo(:, 5) >= 0);
indicesDWPTNegativa = find(valoresSondeo(:, 5) < 0);
Theta = zeros(length(valoresSondeo(:, 5)), 1);
presionVaporActual = zeros(length(valoresSondeo(:, 5)), 1);
% DWPT positivas
Theta(indicesDWPTPositiva, 1) = 1 - (valoresSondeo(indicesDWPTPositiva, 5) + 273.16) / 647.096;
presionVaporActual(indicesDWPTPositiva, 1) = 220640 * exp((647.096 ./ (valoresSondeo(indicesDWPTPositiva, 5) + 273.16)) .* ...
...
Theta(indicesDWPTNegativa, 1) = (valoresSondeo(indicesDWPTNegativa, 5) + 273.16) / 273.16;
...
% presionVaporSaturada_antigua = 6.1086*exp(17.856*valoresSondeo(:, 3)./(245.52 + valoresSondeo(:, 3))); % Calculada con Temp
indicesTempPositiva = find(valoresSondeo(:, 3) >= 0);
indicesTempNegativa = find(valoresSondeo(:, 3) < 0);
Theta = zeros(length(valoresSondeo(:, 3)), 1);
presionVaporSaturada = zeros(length(valoresSondeo(:, 3)), 1);
% Temperaturas positivas
Theta(indicesTempPositiva, 1) = 1 - (valoresSondeo(indicesTempPositiva, 3) + 273.16) / 647.096;
presionVaporSaturada(indicesTempPositiva, 1) = 220640 * exp((647.096 ./ (valoresSondeo(indicesTempPositiva, 3) + 273.16)) .* ...
...
Theta(indicesTempNegativa, 1) = (valoresSondeo(indicesTempNegativa, 3) + 273.16) / 273.16;
...
% plot(presionVaporSaturada_antigua, valoresSondeo(:, 1), '-or', presionVaporSaturada, valoresSondeo(:, 1), '+k');
% plot(presionVaporActual_antigua, valoresSondeo(:, 1), '-or', presionVaporActual, valoresSondeo(:, 1), '+k');

% Fin de la modificación 09/10/2018

RELH = (presionVaporActual./presionVaporSaturada)*100;
% plot(RELH, valoresSondeo(:, 1), valoresSondeo(:, 6), valoresSondeo(:, 1));
% Mixing ratio
MIXR = 621.9907*presionVaporActual./(valoresSondeo(:, 2) - presionVaporActual);
valoresSondeo(:, 7) = MIXR;
...
THTA = (valoresSondeo(:, 3) + 273.16).*power((valoresSondeo(:, 2)/1000), -0.288);
THTE = (valoresSondeo(:, 3) + 273.16).*power((valoresSondeo(:, 2)/1000), -2. / 7. * (1-(.28*.001*MIXR)));
THTV = THTA.*(1+0.00061*MIXR);
valoresSondeo(:, 10) = THTA;
valoresSondeo(:, 11) = THTE;
valoresSondeo(:, 12) = THTV;
...
attGlobales.MaxAltitude = valoresSondeo(1, 1);
